...
def ridge_regression_cv(X, y, cv=5):
    param_grid = {
        "alpha": [0.1, 1.0, 10.0],  # Valores de alpha a probar
        "fit_intercept": [True, False],
        "solver": ["auto", "svd", "cholesky", "sag"]  # Solvers a probar
    }
    model = Ridge()
    grid_search = GridSearchCV(model, param_grid=param_grid, cv=cv)
    grid_search.fit(X, y)
    best_model = Ridge(**grid_search.best_params_)
    best_model.fit(X, y)
...
def ridge_regression_tts(X, y, test_size=0.2, random_state=42):
    param_grid = {
        "alpha": [0.1, 1.0, 10.0],  # Valores de alpha a probar
        "fit_intercept": [True, False],
        "solver": ["auto", "svd", "cholesky", "sag"]  # Solvers a probar
